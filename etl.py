...
def Upload_to_mongoDB(channel_id):
    ...
    return "Extracting Datas"
    

#Creating tables and feeding datas into SQL
def Insert_channels_sql(giv_ch):
 ...
 #Preparing as dataframes using pandas
    ...
    for catch in Yt_collection.find({},{"Channel Table":1,"_id":0}):
        s_name=catch["Channel Table"]['channel_name']
        if s_name == giv_ch:
...
def Insert_videos_sql(giv_ch):
    ...
    for catch in Yt_collection.find({},{"Video Table":1,"_id":0}):
        for i in range (len(catch["Video Table"])):
            vid_ch_name=catch['Video Table'][i]['Channel_Name']
            if vid_ch_name== giv_ch:
...
def insert_tables(giv_ch):
    Insert_channels_sql(giv_ch)
    Insert_videos_sql(giv_ch)
    return "Transfering datas......"
...
with tab1:
        ...
        for Given_id in channel_id:
            ...
            if Given_id not in ch_id_list:
                    ...
                    with st.spinner('Please wait...........'):
                        time.sleep(5)
                        st.success('Channel datas collected.')
    ...
    arr_ch_name=[]
    for get_ch in Yt_collection.find({},{"Channel Table":1,"_id":0}):
        temp_ch=get_ch['Channel Table']['channel_name']
        if temp_ch not in arr_ch_name:
                arr_ch_name.append(temp_ch)
        else:
                continue
            
    GV_channel = st.selectbox("",arr_ch_name,label_visibility="collapsed")

    if st.button("Transfer"):
         with st.spinner('Please wait...........'):
            Push_tables=insert_tables(GV_channel)
            st.success(Push_tables)           
            time.sleep(20)
            st.success('Datas Transfered.Please check insights tab.')
...
with tab2:
    ...
    Ftr_type = st.radio("",[":rainbow[All channels]", "***Individual***"],horizontal=True)

    if Ftr_type=="***Individual***":
        DD_sql= "SELECT channel_name FROM youtube_channels.Channel_datas ORDER BY channel_name ASC"
        mycursor.execute(DD_sql)
        mydb.commit()
        DD_ch_name=mycursor.fetchall()
        Ch_names=[]
        for i in DD_ch_name:
            Ch_names.append(*i)
        Slt_channel = st.selectbox("",Ch_names,label_visibility="collapsed")

        Q1 = st.selectbox("Select the commands below:",
                        ...
                        'Average duration of all videos in the channel'),index=None,placeholder="Select",label_visibility="collapsed")
        ...
        if Q1=='Names of all the videos':
            with st.spinner('Please wait...........'):
                time.sleep(5)
                st.success('Here your results.Fuiyoh!...')
            L1 = """SELECT Video_name,channel_name FROM youtube_channels.Video_datas WHERE channel_name=%s """
            mycursor.execute(L1,(Slt_channel,))
            mydb.commit()
            M1=mycursor.fetchall()
            st.write(pd.DataFrame(M1,columns=["video name","Channel Name"],index=pd.RangeIndex(start=1,stop=len(M1)+1,name="S.no")))

        if Q1=='Top 10 most viewed videos':
            with st.spinner('Please wait...........'):
                time.sleep(5)
                st.success('Here your results.Fuiyoh!...')
            L3 = """SELECT channel_name,video_viewcount,video_name FROM youtube_channels.Video_datas WHERE channel_name=%s ORDER BY video_viewcount DESC limit 10"""
            mycursor.execute(L3,(Slt_channel,))
            mydb.commit()
            M3=mycursor.fetchall()
            st.write(pd.DataFrame(M3,columns=["Channel name","Total Views","video name",],index=pd.RangeIndex(start=1,stop=len(M3)+1,name="S.no")))

        if Q1=='Comments count for each video':
            with st.spinner('Please wait...........'):
                time.sleep(5)
                st.success('Here your results.Fuiyoh!...')
            L4 = """SELECT channel_name,video_commentscount,video_name FROM youtube_channels.Video_datas WHERE channel_name=%s ORDER BY video_commentscount DESC"""
            mycursor.execute(L4,(Slt_channel,))
            mydb.commit()
            M4=mycursor.fetchall()
            st.write(pd.DataFrame(M4,columns=["Channel name","Total comments","video name"],index=pd.RangeIndex(start=1,stop=len(M4)+1,name="S.no")))

        if Q1=='Videos with highest number of likes':
            with st.spinner('Please wait...........'):
                time.sleep(5)
                st.success('Here your results.Fuiyoh!...')
            L5 = """SELECT channel_name,video_likecount,video_name FROM youtube_channels.Video_datas WHERE channel_name=%s ORDER BY video_likecount DESC"""
            mycursor.execute(L5,(Slt_channel,))
            mydb.commit()
            M5=mycursor.fetchall()
            st.write(pd.DataFrame(M5,columns=["Channel name","Total likes","video name"],index=pd.RangeIndex(start=1,stop=len(M5)+1,name="S.no")))

        if Q1=='videos published  in the year 2022':
            with st.spinner('Please wait...........'):
                time.sleep(5)
                st.success('Here your results.Fuiyoh!...')
            L6 = """SELECT channel_name,video_name,video_publishedat
            FROM youtube_channels.Video_datas 
            WHERE YEAR(video_publishedat) = 2022 AND channel_name=%s
            ORDER BY DATE(video_publishedat) ASC"""
            mycursor.execute(L6,(Slt_channel,))
            mydb.commit()
            M6=mycursor.fetchall()
            st.write(pd.DataFrame(M6,columns=["Channel Name","videos posted on 2022","TIME"],index=pd.RangeIndex(start=1,stop=len(M6)+1,name="S.no")))  

        if Q1=='Average duration of all videos in the channel':
            with st.spinner('Please wait...........'):
                time.sleep(5)
                st.success('Here your results.Fuiyoh!...')
            L7 = '''SELECT channel_name,sec_to_time(AVG(video_duration))
            FROM youtube_channels.Video_datas
            WHERE channel_name=%s'''
            mycursor.execute(L7,(Slt_channel,))
            mydb.commit()
            M7=mycursor.fetchall()
            st.write(pd.DataFrame(M7,columns=["Channel Name","Average duration"],index=pd.RangeIndex(start=1,stop=len(M7)+1,name="S.no")))     
        ...
        if Q=='What are the top 10 most viewed videos and their respective channels?':
            with st.spinner('Please wait...........'):
                time.sleep(5)
                st.success('Here your results.Fuiyoh!...')
            C11 = "SELECT channel_name,video_viewcount,video_name FROM youtube_channels.Video_datas ORDER BY video_viewcount DESC limit 10"
            mycursor.execute(C11)
            mydb.commit()
            E11=mycursor.fetchall()
            st.write(pd.DataFrame(E11,columns=["Channel name","Total Views","video name",],index=pd.RangeIndex(start=1,stop=len(E11)+1,name="S.no")))
...
YouTube, as one of the largest platforms for video content, generates an enormous volume of data on a daily basis. This essay aims to delve into the 
process of extracting data from the Google Cloud using the YouTube Data API with the help of Python, saving it in MongoDB, transferring it from MongoDB 
to SQL, and using Streamlit to build a website for data manipulation and interaction.

**Data Extraction**

The extraction process typically involves identifying the specific data elements required, such as video metadata, view counts, likes, comments, and 
other relevant information. Python's rich ecosystem of libraries, including requests and google-api-python-client, facilitates the interaction with the 
YouTube Data API, enabling the retrieval of data in a structured and organized manner.

**Data Storing**

Once the data is extracted from the YouTube platform using the YouTube Data API, the next step involves storing this data in a suitable database for 
further processing and analysis. MongoDB, a popular NoSQL database, offers a flexible and scalable solution for storing unstructured or semi-structured 
data, making it an ideal choice for accommodating the diverse nature of YouTube data.

**Data Transfering**

In some scenarios, the need may arise to transfer the data from MongoDB, a NoSQL database, to a traditional SQL database for various reasons such as 
integration with existing systems, standardized querying, or relational data modeling. This transition involves a careful consideration of data mapping, 
transformation, and migration strategies to ensure the integrity and consistency of the data throughout the transfer process.

**BUild a Website**

After the data has been extracted, stored, and transferred, the next phase involves leveraging Streamlit, a popular open-source framework to create a 
website for interactive data visualization, manipulation, and exploration. Streamlit's intuitive and user-friendly interface, coupled with its seamless 
integration with Python, makes it an ideal choice for rapidly developing and deploying data-driven web applications.

**Data Manipulation**

To achieve this, Streamlit offers a range of input components, such as text inputs, dropdowns, and sliders, to capture user queries or parameters. 
These inputs can be seamlessly integrated with Python code to formulate SQL queries dynamically based on user input. The retrieved data from the SQL 
database can be further processed, transformed, and presented back to the user through interactive visualizations or tabular representations, enabling a 
rich and immersive data exploration experience.
...
In conclusion, the process of YouTube data harvesting and warehousing, encompassing the extraction of data from the Google Cloud 
using the YouTube Data API with Python, saving it in MongoDB, transferring it to SQL, and building a website using Streamlit for 
interactive data manipulation, embodies a multifaceted and intricate workflow. By leveraging the capabilities of Python, MongoDB, SQL, and Streamlit, 
organizations and data enthusiasts can harness the wealth of information available on YouTube to derive valuable insights, drive informed decision-making, 
and unlock the potential of big data in the digital age.
